...
for colum in stat_names:
    for suffix in ["", "_opponent"]:
        data_aggregated[f"{colum}{suffix}_per_minute"] = data_aggregated[f"{colum}{suffix}"] / data_aggregated["total_time"]
        data_aggregated[f"{colum}{suffix}_per_fight"] = data_aggregated[f"{colum}{suffix}"] / data_aggregated["num_fight"]
...
# %%
_ = simulate_bets(min_confidence=0.01, max_diff=0.1, max_bet=10000, print_info=True)
