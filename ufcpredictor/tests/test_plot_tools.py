...
class TestPredictionPlots(unittest.TestCase):
    ...
    def test_show_fight_prediction_detail(self, mock_subplots):
        ...
        PredictionPlots.show_fight_prediction_detail(
            model=mock_model,
            data=(X1, X2, Y, odds1, odds2, fighter_names, opponent_names),
            print_info=False,
            show_plot=True,
        ...
        earnings_progress = plot_args[1]  # (np.cumsum(earning_progress) - np.cumsum(invest_progress)) / np.cumsum(invest_progress) * 100

        # Check if investment and earnings values match expected values
        expected_invest_progress = np.cumsum(expected_invest)  # bet values are expected to be 10 for each
        expected_earning_progress = (
            np.cumsum(expected_earnings) - expected_invest_progress
        ) / expected_invest_progress * 100
    ...
    def test_show_fight_prediction_detail_from_dataset(self, mock_subplots):
        ...
        mock_dataset.get_fight_data_from_ids.assert_called_once_with(["fight_1", "fight_2"])
